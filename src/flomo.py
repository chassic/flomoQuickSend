...
def sendFlomo(msg, url):
    if msg.strip().strip('\n') == '#inbox/' + datetime.datetime.now().strftime('%Y-%m-%d'):
        return '';
...
def initTextInput(textInput):
    textInput.delete(1.0,tk.END)
    textInput.insert(1.0, '#inbox/' + datetime.datetime.now().strftime('%Y-%m-%d'))
    textInput.insert(tk.INSERT, '\n')
    textInput.focus
    textInput.mark_set('insert', '2.0')
...
class _Main:  # 调用SysTrayIcon的Demo窗口
    ...
    def main(s):
        ...
        # 放置控件
        s.textInput = tk.Text(s.root,height=int(screenheight/30) - 10,width=int(screenwidth/30) + 1,relief='groove')
        s.textInput.grid(row=0, column=0, padx=4, pady=2)

        initTextInput(s.textInput)

        s.sendButton = tk.Button(s.root, text="发送", command=s.helloCallBack, width=8, height=2)
        s.sendButton.grid(row=1, column=0, padx=2, pady=2)
        ...
        s.isshow = 1;
        keyboard.add_hotkey('windows+space', s.gobal_callback)
        #keyboard.add_hotkey('ctrl+alt+h', self.hide)

        cf = configparser.ConfigParser()
        cf.read("flomo.conf")
        for configItem in cf.items('config'):
            if (configItem[0] == 'posturl'):
                s.posturl = configItem[1]

        s.root.mainloop()

    def helloCallBack(s):
        print(sendFlomo(s.textInput.get("0.0", "end"), s.posturl))
        print(s.textInput.get("0.0", "end"))
        initTextInput(s.textInput)

    def switch_icon(s, _sysTrayIcon, icon='flomo.ico'):
    ...
    def gobal_callback(self):
        if self.isshow == 1:
            sendFlomo(self.textInput.get("0.0", "end"), self.posturl)
            self.global_hide()
            initTextInput(self.textInput)
            self.isshow = 0;
        else:
            self.global_show()
            self.isshow = 1;
    ...
    def Hidden_window(s, icon='flomo.ico', hover_text="flomo"):
